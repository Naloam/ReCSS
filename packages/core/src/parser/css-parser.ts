...
function resolveSelectors(rule: RuleLike): string[] {
  ...
  const parentRule = parent as { parent?: unknown; selector?: string; type?: string }
...
function addDefinition(map: CssParseResult, className: string, definition: ClassDefinition): void {
...
export async function parseCssCode(filePath: string, sourceCode: string): Promise<CssParseResult> {
...
export async function parseCssFiles(filePaths: string[]): Promise<CssParseResult> {
  const entries = await Promise.all(filePaths.map((filePath) => parseCssFile(filePath)))
