...
function isClassBindingDirective(node: unknown): node is {
  ...
  const candidate = node as { arg?: { content?: string }; name?: string; type?: number }
  return candidate.type === 7 && candidate.name === 'bind' && candidate.arg?.content === 'class'
}
...
function collectFromExpression(
  ...
  switch (node.type) {
    ...
    case 'TemplateLiteral': {
      const quasis = Array.isArray(node.quasis) ? (node.quasis as Array<{ value?: { cooked?: string } }>) : []
      ...
      const expressions = Array.isArray(node.expressions) ? (node.expressions as BabelNode[]) : []
    ...
    case 'ArrayExpression': {
      const elements = Array.isArray(node.elements) ? (node.elements as Array<BabelNode | null>) : []
    ...
    case 'ObjectExpression': {
      const properties = Array.isArray(node.properties) ? (node.properties as BabelNode[]) : []
    ...
    default: {
      ...
      for (const child of children) {
        if (child && typeof child === 'object' && 'type' in child) {
          collectFromExpression(expression, child as BabelNode, used, uncertain)
        } else if (Array.isArray(child)) {
          for (const item of child) {
            if (item && typeof item === 'object' && 'type' in item) {
              collectFromExpression(expression, item as BabelNode, used, uncertain)
            }
          }
        }
      }
...
function collectTemplateClasses(templateContent: string, result: SourceScanResult): void {
  ...
  const stack: Array<{ children?: unknown[]; props?: unknown[] }> = [ast as { children?: unknown[] }]
  ...
  while (stack.length > 0) {
    ...
    for (const prop of props) {
      ...
      if (isClassBindingDirective(prop) && typeof prop.exp?.content === 'string') {
        try {
          const expressionAst = parseExpression(prop.exp.content, {
            plugins: ['typescript'],
          }) as unknown as BabelNode
          collectFromExpression(prop.exp.content, expressionAst, result.used, result.uncertain)
        } catch {
    ...
    for (const child of children) {
      if (child && typeof child === 'object') {
        const candidate = child as { branches?: unknown[]; children?: unknown[]; props?: unknown[] }
...
export function parseVueCode(filePath: string, sourceCode: string): SourceScanResult {
...
export async function parseVueFile(filePath: string): Promise<SourceScanResult> {
  try {
