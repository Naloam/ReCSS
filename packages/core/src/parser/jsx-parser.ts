...
function collectClassHelpers(importNode: AstNode, helpers: Set<string>): void {
  ...
  const specifiers = Array.isArray(importNode.specifiers) ? (importNode.specifiers as AstNode[]) : []
  ...
  for (const specifier of specifiers) {
    ...
    if (specifier.type === 'ImportDefaultSpecifier' && (importSource === 'clsx' || importSource === 'classnames')) {
      helpers.add(localName)
    }

    if (specifier.type === 'ImportSpecifier') {
      const imported = specifier.imported as { name?: unknown } | undefined
      if (typeof imported?.name === 'string' && KNOWN_CLASS_HELPERS.has(imported.name)) {
        helpers.add(localName)
      }
    }
  }
...
function collectFromObjectExpression(objectNode: AstNode, used: Set<string>): void {
  const properties = Array.isArray(objectNode.properties) ? (objectNode.properties as AstNode[]) : []
  ...
  switch (expression.type) {
    ...
    case 'TemplateLiteral': {
      ...
      for (const nestedExpression of nestedExpressions) {
        collectExpressionClasses(sourceCode, nestedExpression, used, uncertain, classHelpers)
      }
      return
    }
    case 'ConditionalExpression': {
      ...
      if (consequent) {
        collectExpressionClasses(sourceCode, consequent, used, uncertain, classHelpers)
      }
      if (alternate) {
        collectExpressionClasses(sourceCode, alternate, used, uncertain, classHelpers)
      }
      return
    }
    case 'LogicalExpression': {
      ...
      if (left) {
        collectExpressionClasses(sourceCode, left, used, uncertain, classHelpers)
      }
      if (right) {
    ...
    case 'ArrayExpression': {
      const elements = Array.isArray(expression.elements) ? (expression.elements as Array<AstNode | null>) : []
      for (const element of elements) {
        if (element && element.type !== 'SpreadElement') {
          collectExpressionClasses(sourceCode, element, used, uncertain, classHelpers)
        }
      }
      return
    }
    ...
    case 'CallExpression': {
      ...
      if (callee?.type === 'Identifier' && typeof callee.name === 'string' && classHelpers.has(callee.name)) {
        const args = Array.isArray(expression.arguments) ? (expression.arguments as AstNode[]) : []
        for (const arg of args) {
          if (arg.type !== 'SpreadElement') {
            collectExpressionClasses(sourceCode, arg, used, uncertain, classHelpers)
          }
        }
        return
      }
    ...
    case 'TSTypeAssertion': {
      ...
      if (inner) {
        collectExpressionClasses(sourceCode, inner, used, uncertain, classHelpers)
      }
      return
    }
    default: {
...
function collectFromClassNameAttribute(
  ...
  if (valueNode.type === 'StringLiteral' && typeof valueNode.value === 'string') {
  ...
  collectExpressionClasses(sourceCode, expressionNode, used, uncertain, classHelpers)
}

export function parseJsxCode(filePath: string, sourceCode: string): SourceScanResult {
  ...
  try {
    ...
    walkAst(ast, (node) => {
      ...
      if (node.type === 'JSXAttribute') {
        ...
        if (nameNode?.type === 'JSXIdentifier' && nameNode.name === 'className') {
...
export async function parseJsxFile(filePath: string): Promise<SourceScanResult> {
  try {
