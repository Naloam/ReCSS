...
describe("migrate helpers", () => {
  ...
  it("should ignore class-like substrings inside data urls", () => {
    const names = extractClassNames(
      [
        ".card {",
  ...
  it("should rewrite aliased react factory imports", async () => {
    const root = await mkdtemp(
      resolve(tmpdir(), "recss-core-migrate-react-factory-aliases-"),
    );
  ...
  it("should use named useCssModule accessor for vue module alias rewrites", async () => {
    const root = await mkdtemp(
      resolve(tmpdir(), "recss-core-migrate-vue-use-css-module-alias-"),
    );
